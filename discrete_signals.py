...
def discretized_chebyshev(degree, N, ran=[-1,1]):
    """
    ...
    return jnp.array(Chebyshev(coeffs, [0, 1.], ran)(jnp.linspace(0, 1. - dt, N) + (dt / 2)))

def discretized_chebyshev_basis(max_degree, N, ran=[-1,1]):
    ...
    #return vmap(lambda deg: discretized_legendre(deg, T, N, ran))(jnp.array(list(range(max_degree+1))))

    vals = list(range(max_degree + 1))
    disc_leg_map = map(lambda deg: discretized_chebyshev(deg, N, ran), vals)
...
def gaussian(x, amp, mean, sig):
    ...
    return amp * jnp.exp(-jnp.power(x - mean, 2.) / (2 * jnp.power(sig, 2.)))

def gauss_conv_kernel(amp, sigma, samples, width):
    """constructs a discrete gaussian"""
    return vmap(lambda t: gaussian(t, amp, 0, sigma))(jnp.linspace(-width, width, samples))
...
def get_gaussian_filter(samples, zero_tol=10**-4, kern_sample_rate=1):
    ...
    width = jnp.sqrt(- (2 * jnp.power(1., 2.)) * jnp.log(zero_tol))

    conv_kernel = gauss_conv_kernel(1., 3., samples, width)
    conv_kernel = conv_kernel / jnp.sum(conv_kernel)
...
def get_param_to_signal(leg_order, N):
    ...
    N_filter = int(jnp.ceil( (3 * filter_std) / dt_filter )) # number of samples for the filter
    ...
    # create basis
    signal_basis = discretized_chebyshev_basis(leg_order, N)
    ...
    def param_to_signal(params):
        ...
        leg_linear_combo = jnp.tensordot(params, signal_basis, axes=(1, 0))
